...
def get_params():
    ...
    parser.add_argument('--patch_number', default=9, type=int)  # The Number of Patch P in IPC Loss
    ...
    parser.add_argument('--tag', default='train_MSA_VOC_image', type=str)
...
def main(args):
    ...
    train_dataset = Dataset_with_RGBIMG(data_dir, domain, train_transform, _dataset=args.dataset)
    ...
    if(args.SP_CAM):
        evaluatorA = evaluator_cls_image.evaluator(args.dataset, domain=val_domain, SP_CAM=True, refine_list=[0], th_list=[0.15, 0.2, 0.3, 0.4])
    else:
    ...
    for iteration in range(max_iteration):
        ...
        if(args.SP_CAM):
            logits, logitsmin = model(images, prob, pcm=args.beta, th=args.th)    
        else:
            logits, logitsmin = model(images, pcm=2)
